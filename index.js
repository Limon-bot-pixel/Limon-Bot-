...
async function pairLogin() {
    ...
    if (!conn.authState.creds.registered) {
        ...
        let phone = "8801623442730"   // আপনার নাম্বার
...
async function messageHandler(conn, m) {
    try {
        ...
        if (cmd === "menu" || cmd === "help") {
            await conn.sendMessage(from, { text: "📌 *Limon Bot Menu*\n\n- .ping\n- .owner\n- .id\n\nMore coming soon..." })
        }

        if (cmd === "owner") {
            await conn.sendMessage(from, { text: "👑 Owner: Limon Bbz\n📞 8801623442730" })
        }

        if (cmd === "id") {
            await conn.sendMessage(from, { text: `🆔 Your ID: ${from}` })
        }

        // Plugin Commands
        for (let plug in global.PLUGINS) {
            try {
                global.PLUGINS[plug](conn, msg, text, cmd)
            } catch { }
        }
